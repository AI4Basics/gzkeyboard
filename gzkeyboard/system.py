...
# %% auto 0
__all__ = ['CONTEXT_TIMEOUT', 'InputMode', 'KeyboardConfig', 'HotkeyManager', 'StatusIndicator', 'SystemInputHandler',
           'create_default_config', 'start_keyboard', 'stop_keyboard', 'main']
...
class SystemInputHandler:
    ...
    - Context timeout: if more than CONTEXT_TIMEOUT seconds pass between keys,
      context is reset so that e.g. 's' + (pause) + 's' gives ስስ, not ሥ.
    - Injection guard: synthetic keystrokes from keyboard.write/send are ignored
    ...
    def __init__(self, config: Optional[KeyboardConfig] = None,
        ...
        self._injecting = False     # re-entrancy guard for keyboard.write/send
    ...
    def setup_keyboard_hook(self):
        ...
        try:
            ...
            self.status.show_status("'keyboard' package not installed. Run: pip install keyboard")
            return False
        
        try:
            def on_key_event(event):
                # Skip synthetic keystrokes we injected ourselves
                if self._injecting:
                    return True
                ...
                # Let modifier combinations pass through (Ctrl+C, Alt+L, etc.)
                try:
                ...
                # Context timeout: reset if user paused between keystrokes
                now = time.time()
                if now - self._last_key_time > CONTEXT_TIMEOUT:
                    self.engine.reset()
                ...
                if len(key) == 1 and (key.isalpha() or key in "':.;,?"):
                    ...
                    # Execute the action, guarding against re-entrancy
            ...
            try:
                ...
                if self.config.debug_mode:
                    print("Using suppress=True mode (full intercept)")
            except Exception as e1:
                if self.config.debug_mode:
                    print(f"suppress=True failed ({type(e1).__name__}: {e1}), trying suppress=False...")
                keyboard.hook(on_key_event, suppress=False)
                if self.config.debug_mode:
                    print("Using suppress=False mode (fallback)")
